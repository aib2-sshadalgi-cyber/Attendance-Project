...
export default function ScanAttendance() {
  ...
  useEffect(() => {
    api
      ...
      .then((res) => {
        ...
        const preselect = location.state?.lectureId;
        if (preselect && res.data?.some?.((x) => x.id === preselect)) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-slate-900">Capture attendance</h1>
        <p className="text-slate-600">
          Facial encoding is compared securely with your registrar profile — only unique lecture slots unlock.
        </p>
      </div>
      {feedback && (
        <Banner
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        <div className="rounded-3xl bg-white p-6 shadow-panel lg:col-span-2">
          ...
          <p className="mt-4 text-xs text-slate-500 leading-relaxed">
            Once marked, any additional attempt displays: 👉{' '}
        ...
        <div className="rounded-3xl bg-white p-6 shadow-panel lg:col-span-3">
          <FaceCapture
            disabled={!selected}
            buttonLabel={selected ? 'Capture face & authenticate' : 'Choose a lecture'}
